...
export(float) var walk_speed = 2 
export(float) var sprint_speed = 4
export(int) var acceleration = 8
export(int) var deacceleration = 10
export(float, 0.0, 1.0, 0.05) var air_control = 0.3
export(int) var jump_height = 6
var _speed: float
...
func _process(_delta: float) -> void:
	...
	if Input.is_action_just_pressed("move_jump") && !is_win:
		_is_jumping_input = true
	
	if Input.is_action_pressed("move_sprint") && !is_win:
		_is_sprinting_input = true
		
	# рестар по R или если мы упали вниз
	if Input.is_action_pressed("restart") || global_translation.y < -100:
